...
const NavBar = ({ scrollToSection }) => {
  ...
  return (
    <div className="hidden lg:navbar z-[1000000] h-[90px] bg-white fixed max-w-[968px] p-0 ">
    <div className="navbar-start">
        <a href="/" className="text-[20px] font-title text-[#333333] font-medium my-auto">
            #Oveshek
        </a>
    </div>
    <div className="navbar-center navbar-end">
        <ul className="flex gap-7 items-center">
            {links.map((item) => (
                <li  >
                    <p
                        onClick={() => scrollToSection(`${item.path}`)}
                        className="flex flex-col gap-1 text-[#757575 ] items-center justify-center group cursor-pointer"
                    >
